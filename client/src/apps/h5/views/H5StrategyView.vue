  ...
  <div class="h5-page fade-up strategy-page">
    ...
    <template v-else>
      ...
      <section class="strategy-actions-card">
        <button type="button" class="h5-btn block" @click="handlePrimaryAction">{{ primaryActionLabel }}</button>
    ...
    <H5StickyCta
      ...
      :secondary-label="activeDetailID ? '返回列表' : '去资讯页'"
      @primary="handlePrimaryAction"
      @secondary="activeDetailID ? closeDetail() : router.push('/news')"
    />
  </div>
</template>
...
const stickyTitle = computed(() => activeDetailID.value ? "看完策略简报后，把动作收在最后一个 CTA" : "先选一条观点，再进入单页策略简报");
const stickyDescription = computed(() => isVIPUser.value ? "当前会员状态已同步，可继续阅读和跟踪。" : "开通会员后，可继续解锁更多正文、附件和策略解释。");
const primaryActionLabel = computed(() => activeDetailID.value ? "继续跟踪这条策略" : "查看一条观点");
