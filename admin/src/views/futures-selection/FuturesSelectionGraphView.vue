...
const graphEntityTypeOptions = [
  { label: "期货合约", value: "FuturesContract" },
...
async function fetchRuns() {
  loading.value = true;
  try {
    const data = await listFuturesSelectionRuns({ page: 1, page_size: 12 });
    ...
    ElMessage.error(error?.message || "加载最近期货运行失败");
  } finally {
    loading.value = false;
  }
}
...
<template>
  <FuturesSelectionModuleShell
    title="智能期货事件图谱"
    description="把最近运行的图快照、关联实体和关系查询集中到一个工作台里。即使本地 Neo4j/图服务重启导致旧快照失效，也可以继续从最近运行的关联实体发起子图查询。"
  >
    <template #actions>
    ...
    <div class="graph-layout">
      <div class="card">
        <div class="card-title">最近期货运行</div>
        <el-table
          :data="recentRuns"
          border
          stripe
        ...
        >
          ...
          <el-table-column label="状态" min-width="90">
            <template #default="{ row }">{{ formatFuturesSelectionRunStatus(row.status) }}</template>
          </el-table-column>
        </el-table>
      </div>
        ...
        <template v-else>
          ...
          <el-descriptions v-if="snapshot" :column="2" border size="small" style="margin-top: 12px">
            <el-descriptions-item label="快照域">
              {{ formatFuturesSelectionAssetDomain(snapshot.asset_domain) }}
            </el-descriptions-item>
    ...
    <div class="card" style="margin-top: 12px">
      ...
      <div class="query-grid">
        ...
        <el-form-item label="实体键">
          <el-input v-model="queryForm.entity_key" placeholder="例如 FUTURES_REGIME:BASE 或 IF" />
        </el-form-item>
        <el-form-item label="资产域">
          <el-input v-model="queryForm.asset_domain" placeholder="例如 跨资产（cross）或 期货（futures）" />
        </el-form-item>
        ...
        <el-descriptions :column="1" border size="small">
          <el-descriptions-item label="根实体">
            ...
            <template v-if="subgraph?.entity?.entity_type || queryForm.entity_type">
              / {{ formatFuturesSelectionGraphEntityType(subgraph?.entity?.entity_type || queryForm.entity_type) }}
            </template>
          </el-descriptions-item>
          <el-descriptions-item label="实体域">
            {{ formatFuturesSelectionAssetDomain(subgraph?.entity?.asset_domain || queryForm.asset_domain) }}
          </el-descriptions-item>
