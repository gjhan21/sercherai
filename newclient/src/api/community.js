...
function resolveReadPath(path) {
  return getAccessToken() ? `/community${path}` : `/public/community${path}`;
}
...
export function listCommunityComments(topicID, params) {
  return http.get(resolveReadPath(`/topics/${encodeURIComponent(topicID)}/comments`), { params: buildParams(params) });
}
