...
func (r *InMemoryGrowthRepo) GetStockRecommendationInsight(userID string, recoID string) (model.StockRecommendationInsight, error) {
	...
	return model.StockRecommendationInsight{
		...
		Explanation: model.StrategyClientExplanation{
			ConsensusSummary: "高景气消费龙头仍处于资金与基本面共振区间。",
			ConfidenceReason: "机构调研与资金回流节奏仍支持主线。",
			RiskBoundary:     "跌破关键支撑位且资金回流转负，则主线失效。",
			...
			EvaluationMeta: map[string]any{
				"stock_quant_score": model.StockQuantScore{
					...
					ValueScore:       69.5,
					NewsScore:        74.8,
					NetMFAmount:      1234567.89,
					PeTTM:            27.4,
					PB:               8.1,
					TurnoverRate:     1.9,
					NewsHeat:         3,
					PositiveNewsRate: 0.74,
				},
			},
		},
...
func (r *InMemoryGrowthRepo) GetFuturesStrategyInsight(userID string, strategyID string) (model.FuturesStrategyInsight, error) {
	...
	return model.FuturesStrategyInsight{
		...
		Guidance: guidance,
		Explanation: model.StrategyClientExplanation{
			ConsensusSummary: "近月结构和资金配合仍支持趋势跟踪主线。",
			ConfidenceReason: "基差、库存和持仓线索当前保持同向。",
			InventorySummary: "库存延续去化，短期供需压力缓和。",
			MarketRegime:     "TREND_CONTINUE",
			RiskBoundary:     "若期限结构转弱且流向偏置翻空，则主线失效。",
			Invalidations:    []string{"期限结构转弱", "流向偏置翻空"},
			ScenarioMeta: model.StrategyExplanationScenarioMeta{
				PrimaryScenario: "trend_continue",
			},
			EvaluationMeta: map[string]any{
				"futures_seed": model.StrategyEngineFuturesSeed{
					...
					FlowBias:               0.66,
					CarryPct:               0.84,
					TermStructurePct:       1.15,
					CurveSlopePct:          0.37,
					InventoryLevel:         72.4,
					InventoryChangePct:     -2.1,
					InventoryPressure:      0.42,
					InventoryFocusArea:     "华东",
					InventoryFocusWarehouse:"主交割仓",
					InventoryBrandGradeSummary: "主交割仓库存下降，品牌结构偏紧。",
					BasisTermAlignment:     0.78,
					CrossContractLinkage:   0.63,
					StructureSignalSummary: "近月升水与期限结构保持同向。",
					SpreadPressure:         0.44,
					Regime:                 "TREND_CONTINUE",
				},
			},
		},
