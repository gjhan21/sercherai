...
type GrowthRepo interface {
	...
	SavePatternMatches(matches []model.StockPatternMatch) error
	...
	ListStrategyForecastL3HistoryForTarget(requestUserID string, targetType string, targetKey string, page int, pageSize int) ([]model.StrategyForecastL3HistoryItem, error)
	GetStrategyForecastL3HistoryCompare(requestUserID string, targetType string, targetKey string, leftRunID string, rightRunID string) (model.StrategyForecastL3HistoryCompare, error)
	GetStrategyForecastL3RunReview(runID string, requestUserID string) (model.StrategyForecastL3RunReview, error)
	...
	ListStrategyForecastL3QualitySummaries(targetType string, days int) ([]model.StrategyForecastL3QualitySummary, error)
	ExecuteQueuedStrategyForecastL3Runs(limit int, operatorUserID string) (int, error)
	RunStrategyForecastL3QualityBackfill(limit int, operatorUserID string) (int, error)
	...
	AdminUpsertFuturesInstrumentProfile(item model.FuturesInstrumentProfile) (model.FuturesInstrumentProfile, error)
	AdminGetFuturesInstrumentProfile(productKey string) (model.FuturesInstrumentProfile, error)
	...
	AdminListStockEventClusters(query model.StockEventQuery) ([]model.StockEventCluster, int, error)
	AdminGetStockEventCluster(id string) (model.StockEventCluster, error)
	AdminUpsertStockEventCluster(cluster model.StockEventCluster) (model.StockEventCluster, error)
	AdminCreateStockEventReview(review model.StockEventReview) (model.StockEventReview, error)
	...
	AdminGetUsersByEmailRule(ruleType string) ([]model.AdminUser, error)
	AdminRecordEmailLog(userID string, email string, subject string, body string, ruleType string, status string, errMsg string) error
