...
func Register(r *gin.Engine) {
	...
	v1 := r.Group("/api/v1")
	{
		...
		adminDataSources.Use(middleware.AuthRequired(cfg.JWTSecret), middleware.RoleRequired("ADMIN"))
		{
			...
			adminDataSources.GET("/governance/capabilities", middleware.PermissionRequired(db, "data_source.view"), adminGrowthHandler.ListMarketProviderCapabilities)
			adminDataSources.GET("/governance/routing-policies", middleware.PermissionRequired(db, "data_source.view"), adminGrowthHandler.ListMarketProviderRoutingPolicies)
			adminDataSources.PUT("/governance/routing-policies/:policy_key", middleware.PermissionRequired(db, "data_source.edit"), adminGrowthHandler.UpdateMarketProviderRoutingPolicy)
		...
		adminStocks.Use(middleware.AuthRequired(cfg.JWTSecret), middleware.RoleRequired("ADMIN"))
		{
			...
			adminStocks.POST("/master/sync", middleware.PermissionRequired(db, "market.edit"), adminGrowthHandler.SyncStockInstrumentMaster)
			adminStocks.POST("/quotes/sync", middleware.PermissionRequired(db, "market.edit"), adminGrowthHandler.SyncStockQuotes)
			adminStocks.POST("/daily-basic/sync", middleware.PermissionRequired(db, "market.edit"), adminGrowthHandler.SyncStockDailyBasics)
			adminStocks.POST("/moneyflow/sync", middleware.PermissionRequired(db, "market.edit"), adminGrowthHandler.SyncStockMoneyflows)
			adminStocks.POST("/news/sync", middleware.PermissionRequired(db, "market.edit"), adminGrowthHandler.SyncStockNewsSource)
			adminStocks.POST("/backfill", middleware.PermissionRequired(db, "market.edit"), adminGrowthHandler.BackfillStockMarketData)
