...
class MarketSeedLoader:
    ...
    def _parse_seed(self, item: dict[str, Any]) -> MarketSeed:
        return MarketSeed(
            ...
            momentum1=_as_float(item.get("momentum1")),
            momentum2=_as_float(item.get("momentum2")),
            momentum3=_as_float(item.get("momentum3")),
            consecutive_down_days=_as_int(item.get("consecutive_down_days")),
            candle_body_pct=_as_float(item.get("candle_body_pct")),
            lower_shadow_pct=_as_float(item.get("lower_shadow_pct")),
            upper_shadow_pct=_as_float(item.get("upper_shadow_pct")),
            is_bullish=_as_bool(item.get("is_bullish")),
            is_doji=_as_bool(item.get("is_doji")),
            is_engulfing_bullish=_as_bool(item.get("is_engulfing_bullish")),
            deviation_ma5=_as_float(item.get("deviation_ma5")),
            deviation_ma10=_as_float(item.get("deviation_ma10")),
            deviation_ma20=_as_float(item.get("deviation_ma20")),
            deviation_ma60=_as_float(item.get("deviation_ma60")),
            volume_20d_min_rank=_as_int(item.get("volume_20d_min_rank")),
            volume_contraction_days=_as_int(item.get("volume_contraction_days")),
            is_limit_up=_as_bool(item.get("is_limit_up")),
            lu_time_rank=_as_int(item.get("lu_time_rank")),
            seal_order_ratio=_as_float(item.get("seal_order_ratio")),
            limit_up_days=_as_int(item.get("limit_up_days")),
            is_opened=_as_bool(item.get("is_opened")),
            is_natural_limit=item.get("is_natural_limit") is not False,
            on_top_list=_as_bool(item.get("on_top_list")),
            top_net_amount=_as_float(item.get("top_net_amount")),
